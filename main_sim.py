...
def forecast(X, DX, d, p, model_path, forecast_path, until=1000, device='cpu'):
    ...
    fc = torch.from_numpy(X[:, p:-1]) + torch.t(pred)
    prev = torch.from_numpy(X[:, :p+1])

    out = torch.cat((prev, fc), dim=1)
    ...
    for _ in tqdm(range(until)):
        ...
        new_x = out[:, -1:] + torch.t(new_dx)
        out = torch.cat((out, new_x), dim=1)
...
if __name__ == "__main__":
    ...
    data_path = 'data/data_sim_small.npy'
    model_path = 'model/sm_sim.pt'
    forecast_path = 'output/sm_sim2.pickle'
    ...
    # forecast(X, DX, d, p, model_path, forecast_path, until)
