...
#=========================================================================================
# Part I 
# simulation for stationary case
...
#Version II
#==============================================================================================================
#non-stationary simulation; time varying spatial weight matrix version2
#==============================================================================================================
# The simulation only consider time lag eequals 1
library(R.matlab)
library(vars)
library("matrixStats")
set.seed(18)
...
# function to creat spatial weight matrix; where alpha control the magnitute of shape function,i.e. alpha*log() is the shape function
generate_w = function(n.index,alpha){
  W = matrix(0,nrow = length(n.index),ncol = length(n.index))
  for (i in 1:length(n.index)){
    for (j in 1:length(n.index)) {
...
generate_one_step = function(x.t_1, W.t_1,mu.t_1,mu.t,epsilon.t_t){
  n = dim(W.t_1)[1]
  x.t = matrix(0,nrow = n,ncol = 1)
  x.t = W.t_1 %*% (x.t_1 - mu.t_1) + mu.t + epsilon.t_t
  return(x.t)
}

# The function aims to simulate all data. It iteratively apply generate_one_step function
generate_sim = function(x_1,alpha.bound,T,mu,epsilon){
  x = cbind(x_1)
  n = dim(x_1)[1]
  for(t in 2:T){
    ...
    tem = matrix(0,nrow = n,ncol = 1)
    tem = generate_one_step(matrix(x[,ncol(x)],nrow = n),W.t_1,mu[,t-1],mu[,t],epsilon[,t])
    x = cbind(x,tem)
  }
  return(x)
}
...
# create epsilon_t
epsilon_t = matrix(rnorm(30*4000,0,20),nrow = 30)
# initial x_1 at time=1
initial_value = matrix(runif(30,-1,1),nrow = 30,ncol = 1)
x = generate_sim(initial_value,c(0.01,3),3000,mu_t,epsilon_t)
plot(x[10,],type = 'l')
plot(mu_t[30,])